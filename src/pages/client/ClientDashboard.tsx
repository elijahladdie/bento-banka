...
import { mockAccounts, mockTransactions, mockNotifications, formatCurrency, getStatusBadgeClass } from "@/data/mockData";
...
const ClientDashboard = () => {
  ...
  const accounts = mockAccounts.filter((a) => a.ownerId === user?.id);
  const totalBalance = accounts.reduce((s, a) => s + a.balance, 0);
  const notifications = mockNotifications.filter((n) => n.receiverId === user?.id);
  const recentTxns = mockTransactions
    .filter((t) => accounts.some((a) => a.id === t.fromAccount || a.id === t.toAccount))
    .slice(0, 5);
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bento-card">
            ...
            <button onClick={() => navigate("/client/profile")} className="text-xs text-primary hover:underline">Edit Profile →</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bento-card lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {accounts.map((a) => (
                <div key={a.id} className="rounded-xl border border-border bg-secondary/50 p-4 hover:bg-secondary transition-colors cursor-pointer" onClick={() => navigate("/client/accounts")}>
